...
```bash
# 方式 A：從 PyPI 安裝（附帶 `loci` 和 `loci-mcp` 兩個命令）
pip install "loci-rag[pdf,docx]"   # 可選 extras：PDF 含表格、Word 文件
...
| `[[sources]]` | 文件目錄列表，遞迴掃描 `.md` / `.txt` / `.html` / `.org`（裝了 extras 還支援 `.pdf` / `.docx`） |
